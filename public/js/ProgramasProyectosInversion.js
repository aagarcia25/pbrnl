...
function GetAllPPI() {
    repository.ProgramasProyectosInversion.GetAllPPI({ejercicio_fiscal: $("#select_ef").val()})
        .then(ResponseGetAllPPI);
}

function ResponseGetAllPPI(response) {
    if (!response.error) {
...
function GetAllCountPPI(){
    repository.ProgramasProyectosInversion.GetAllCountPPI({ejercicio_fiscal: $("#select_ef").val()})
...
function GetPPI(id_secretaria) {
    var request = {
        "id_secretaria": id_secretaria,
        ejercicio_fiscal: $("#select_ef").val()
    }
    repository.ProgramasProyectosInversion.GetPPI(request)
        .then(ResponseGetPPI);
}

function ResponseGetPPI(response) {
    if (!response.error) {
        DestroyDataTable("table");
        $("#table > tbody > tr").remove();
        ppis = response.data;
...
function GetInfoComponentesPPI(request) {
    repository.ProgramasProyectosInversion.GetInfoComponentesPPI(request)
        .then(ResponseGetInfoComponentesPPI);
}

function ResponseGetInfoComponentesPPI(response){
    if (!response.error) {
        if (response.data.length > 0){
...
function GetComponentesPPI(request) {
    repository.ProgramasProyectosInversion.GetComponentesPPI(request)
        .then(ResponseGetComponentesPPI);
}

function ResponseGetComponentesPPI(response) {
    if (!response.error) {
...
function GuardarComponente(){
    $("#form_componente").on("submit", function(event) {
        ...
        Func_DespliegaConfirmacion("Guardar", "¿Deseas guardar la información del actualizada del componente?", "question", "Aceptar", "Cancelar", function(response) {
            if (response) {
                Func_Cargando();
                repository.ProgramasProyectosInversion.EditComponentePPI(request)
                    .then(ResponseEditComponentePPI);
            }
        });
...
function ResponseEditComponentePPI(response) {
    if (!response.error) {
...
function GuardarActualizarPP(){
    $("#form_pp").on("submit", function(event) {
        ...
        Func_DespliegaConfirmacion("Guardar", "¿Deseas actualizar la información del programa presupuestario?", "question", "Aceptar", "Cancelar", function(response) {
            if (response) {
                Func_Cargando();
                repository.ProgramasProyectosInversion.EditProgramaPresupuestoInversion(request)
                    .then(ResponseEditProgramaPresupuestoInversion);
            }
        });
...
function ResponseEditProgramaPresupuestoInversion(response) {
    if (!response.error) {
