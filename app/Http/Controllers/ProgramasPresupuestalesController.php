...
class ProgramasPresupuestalesController extends BaseController
{
    public function all()
    {
        $query = "SELECT A.* FROM PROGRAMATICO AS A INNER JOIN SECRETARIAS AS B ON A.idSecretaria = B.idSecretaria WHERE A.idClasificacion IN ('PP') ORDER BY A.Consecutivo;";
    ...
    {
        $query = "SELECT * FROM
        (SELECT COUNT(*) AS Programas FROM PROGRAMATICO AS A INNER JOIN SECRETARIAS AS B ON A.idSecretaria = B.idSecretaria WHERE A.idClasificacion IN ('PP') ORDER BY A.Consecutivo) AS Programas,
        (SELECT COUNT(*) AS Componentes FROM PROGRAMATICO_COMP AS A INNER JOIN UNIDADES AS B ON A.idUA = B.idUnidad AND A.idSecretaria = B.idSecretaria) AS Componentes;";
    ...
    public function info(Request $request)
    {
        $query = "SELECT B.idSecretaria, B.Descripcion 'Descripcion_Secretaria', C.Descripcion 'Descripcion_Topologia', A.idClasificacion, A.Anticorrupcion, A.idTipologia, C.Descripcion, A.Consecutivo, A.DescripcionPrograma,
                ...
                WHERE A.idClasificacion IN ('PP') AND A.idSecretaria = '$request->id_secretaria' AND A.idObjetivoPED = '$request->id_objetivo' AND A.idClasificacion = '$request->id_clasificacion' AND A.Consecutivo = '$request->consecutivo' ORDER BY A.Consecutivo;";
        $informacion = DB::select($query);
